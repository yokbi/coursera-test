#!/usr/bin/env bash
#
# run-mac-apple-silicon.sh — Alışkanlık Takibi'ni Apple Silicon Mac'te (arm64) tek komutla
# ayağa kaldırır. Intel Mac için run-mac-intel.sh kullanın.
#
# Kullanım:
#   ./run-mac-apple-silicon.sh              # tam yığın: PostgreSQL + API + frontend
#   ./run-mac-apple-silicon.sh --frontend   # SADECE frontend testleri + derleme
...
if [[ "$(uname -s)" == "Darwin" && "$(uname -m)" != "arm64" ]]; then
  warn "Bu makine $(uname -m) görünüyor; ./run-mac-intel.sh önerilir."
  echo
fi

need_node() {
  command -v node >/dev/null 2>&1 || fail "Node.js bulunamadı.
  https://nodejs.org (Apple Silicon için macOS arm64 .pkg) veya: brew install node@20"
...
if [[ "${1:-}" == "--frontend" ]]; then
  ...
  npm run build
  echo
  ok "Frontend doğrulandı. Tam yığın için argümansız çalıştırın: ./run-mac-apple-silicon.sh"
  exit 0
fi
...
command -v docker >/dev/null 2>&1 || fail "Docker bulunamadı.
  Docker Desktop kurun (Apple Silicon sürümü): https://docker.com/products/docker-desktop
  Sadece frontend'i denemek isterseniz: ./run-mac-apple-silicon.sh --frontend"
...
command -v dotnet >/dev/null 2>&1 || fail ".NET SDK bulunamadı.
  .NET 8 SDK kurun — Apple Silicon için ${BOLD}Arm64${RESET} yapısını seçin:
